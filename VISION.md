...
| 4 | **Construir** | **Símbolos fantasma resolubles estáticamente** (== `graph --gate`) — hecho AST, único gate real hoy. *El "recibo de ejecución" (hash+timestamp de una corrida verde) queda **propuesto, NO construido**: exige un segundo tipo de evento en la consola (regla 5) y roza el auto-reporte —el modelo elige qué correr—. No es gate hasta pasar por [SCOPE.md](SCOPE.md).* | Rojo/verde como prueba de corrección (gameable: verde tautológico `assert x is not None`). No-ablandamiento de tests (`check`, ya PROXY) | ¿Es **este** test el correcto? ¿Prueba lo que querías? ¿La cobertura basta? | **gate parcial** + juicio |
...
- **La fase 4 vuelve.** El loop generar → verificar → reparar (la forja, hoy en forja-kit) es el corazón.
  Su gate real HOY son los **símbolos fantasma** (`graph --gate`, hecho AST). El **recibo de ejecución**
  (hash + timestamp de una corrida verde) sería más fuerte contra "dije listo sin ejecutar", pero es un
  **candidato pendiente de [SCOPE.md](SCOPE.md)**: exige un segundo tipo de evento en la consola (regla 5)
  y probablemente es auto-reporte. **Decisión de Marcos, no zanjada aquí.**
