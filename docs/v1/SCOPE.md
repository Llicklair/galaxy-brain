# galaxy-brain — Scope (v1 — FROZEN, 2026-07-29)

> **Este es el alcance de v1 y está congelado.** El vigente está en [SCOPE.md](../../SCOPE.md), con
> la lista explícita de lo que deja de estar dentro y el inventario que lo justifica. Este documento
> se conserva entero: su roadmap y sus anti-goals siguen siendo la referencia histórica de v0.1–v2.0.
...
(see [docs/research-report.md](../research-report.md)) says elaborate scaffolds add little and burn
...
| **v2.0** | universe brain | **Credibility, mechanical**: an automated A/B rig (`eval/run.js` — stage → two arms → blind verifier → cost table, no hand-orchestration) and a self-verifying toolchain (a `node --test scripts/tests/*.test.js` suite over every `scripts/*.js`, so the plugin's own tooling has a regression net). Decided from v1.0 field experience: the four A/B tasks were run by hand, and the new scripts shipped without tests. | The 4-task rig regenerates its result table from one command, and `scripts/` tests are green in CI — "it multiplies capability" is a number a machine reproduces, not a table a human assembles |
...
The ecosystem scan lives in [docs/ecosystem-ideas.md](../ecosystem-ideas.md) — what neighboring
