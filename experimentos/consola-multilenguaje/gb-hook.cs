...
internal class StartupHook
{
    ...
    private static string BuildRecord(Exception ex)
    {
        ...
        string sessionId = Environment.GetEnvironmentVariable("GB_SESSION_ID") ?? "unknown";
        JsonField(sb, "session_id", sessionId);
    ...
    private static void AppendProcessInfo(StringBuilder sb)
    {
        ...
        // Parent process ID for cross-language correlation.
        try
        {
            using var proc = System.Diagnostics.Process.GetCurrentProcess();
            // .NET 5+ has Environment.ProcessId; parent needs Process class
            // On older runtimes this may throw -- catch silently.
            string gbPpid = Environment.GetEnvironmentVariable("GB_PPID");
            if (!string.IsNullOrEmpty(gbPpid))
                JsonField(sb, "ppid", int.Parse(gbPpid));
        }
        catch { /* best-effort */ }
