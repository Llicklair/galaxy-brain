...
Sin servidor, sin websockets, sin proceso extra: si el watch se para, el mapa
dice su hora y se queda quieto — un mapa parado con fecha no miente.
...
def render(foto, refresco=0):
    ...
    partes.append(
        ...
        "</style></head><body>")
    partes.append("<h1>gb who — quien toca que, ahora mismo</h1>")
    partes.append(
        "<div class='meta'>base %s · generado %s%s · derivado de los worktrees "
        "(arbol sucio + consola + commit reciente) — nadie declara nada</div>"
        % (_e(foto.get("base") or "?"), time.strftime("%H:%M:%S"),
           (" · refresco %ds" % refresco) if refresco else ""))

    if foto.get("motivo"):
        partes.append("<p class='vacio'>%s</p>" % _e(foto["motivo"]))
    elif not foto.get("agentes"):
        partes.append(
            "<p class='vacio'>Nadie esta tocando nada ahora mismo. Quien "
